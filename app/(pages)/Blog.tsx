...
export default function Blog() {
  ...
  const { data: dbBlogPosts, isLoading: dbLoading } = useQuery<DBBlogPost[]>({
    queryKey: ['/api/blog'],
    queryFn: async () => {
  ...
  const postsToShow = usingMarkdown && markdownPosts.length > 0 
    ? markdownPosts 
    : (dbBlogPosts || []);
  const isLoading = usingMarkdown ? false : dbLoading;
                ...
                >
                  <div className="relative h-48 overflow-hidden">
                    <img
                      src={
                        isMarkdown 
                          ? (post as MarkdownBlogPost).imageUrl || '/images/e-bikes/keteles-fat-26.png'
